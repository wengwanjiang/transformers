...
from .modeling_draft import LlamaForCausalLMEagle3
from .eagle3_utils import padding

from transformers import PreTrainedModel
from transformers import AutoModelForCausalLM
from transformers import AutoEagle3DraftModel

from .configuration_eagle3 import Eagle3Config


class Eagle3Model(PreTrainedModel):
    """
    ...
    def __init__(self, config: Eagle3Config):
        ...
        self.draft_model = AutoEagle3DraftModel.from_config(config.draft_config)
        
        self.draft_model.load_embedding(config.draft_config.target_model_path,\
                                        config.draft_config.target_model_embed_tokens_key)
        self.draft_model.freeze_embedding()
        self.draft_model.load_vocab_mapping(config.draft_config.vocab_mapping_path)
        

        self.target_model = AutoModelForCausalLM.from_pretrained(config.target_config)

        self.target_model.eval()
        for param in self.target_model.parameters():
            param.require_grad = False
        self.length = config.length
        self.print_freq, self.step = 100, 0
    
    def get_input_embeddings(self):
        return self.draft_model.embed_tokens

    def set_input_embeddings(self, value):
        self.draft_model.embed_tokens = value
    ...
    def forward(
        ...
        position_ids: Optional[torch.Tensor] = None, hidden_state = None, target = None
    ) -> Tuple[List[torch.Tensor], List[torch.Tensor], List[torch.Tensor]]:
        # print(input_ids.sum(), loss_mask.sum(), attention_mask.sum())
        # print(input_ids.shape, loss_mask.shape, attention_mask.shape)
        # exit(0)
        """
        ...
        for idx in range(self.length):
            ...
            # print(f'target_p.shape = {target_p.shape}')
            # print(f'out_logp.shape = {out_logp.shape}')
            # exit(0)
        ...
        self.step += 1
        if self.step % self.print_freq == 0 and sum(acces) > 1e-5:
            plosses1 = round((sum(plosses) / len(plosses)).item(), 8)
            acces1 = [round(u, 5) for u in acces]
            print(f'\n Step: {self.step}, plosses = {plosses1} \t acces = {acces1} \n')
    ...
    def save_pretrained(
        ...
        **kwargs,
        ...
        draft_state_dict = {
            k[len('draft_model.'):]: v for k, v in self.state_dict().items() if 'draft_model.' in k
        }
        
        super().save_pretrained(save_directory, is_main_process, draft_state_dict, save_function, push_to_hub, \
